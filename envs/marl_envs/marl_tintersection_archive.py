    ...
    def take_step(self, high_level_action, exited, num_of_vehicles):
        ...
        #             # self.vehicle_take_action(vehicle, low_level_action)
            ...
            if len(self.right_vehicle_queue) > vehicles_to_exit[1]:
                ...
                agentID = self.get_agentID(side, vehicles_to_exit[1])
                if self.before_box_range(front_of_remaining_vehicles, side) == True:
                    self.agents_steering[agentID] = 0.
                    self.agents_throttle[agentID] = -1.
                    # self.rest_should_stop = True
                else:
        ...
        ################################# LEFT ONLY #################################
            ...
            # 3. assign actions for vehicles on the opposite side
            front_of_opposite_remaining_vehicles = self.right_vehicle_queue[0]
            agentID = self.get_agentID('right', 0)
            print("-===========================================================")
            print(self.before_box_range(front_of_opposite_remaining_vehicles, 'right') == True)
            print("-===========================================================")
            if self.before_box_range(front_of_opposite_remaining_vehicles, 'right') == True:
                self.agents_steering[agentID] = 0.
                self.agents_throttle[agentID] = -1.
            else:
                action = self.agents_policy[agentID].act(agentID)
